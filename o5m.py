...
class OsmData(object):
	...
	def LoadFromO5m(self, fi):
		...
		content.funcStoreNode = self.StoreNode
		content.funcStoreWay = self.StoreWay
		content.funcStoreRelation = self.StoreRelation
		content.funcStoreBounds = self.StoreBounds
		content.funcStoreIsDiff = self.StoreIsDiff
		content.DecodeHeader()

		eof = False
		while not eof:
			eof = content.DecodeNext()
